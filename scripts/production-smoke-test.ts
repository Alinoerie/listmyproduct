 ...
 *
 * Usage:
 *   SMOKE_BASE_URL=https://listmyproduct.vercel.app pnpm test:smoke:prod
 *
 * With pipeline verification (needs .env.local + test account):
 *   bash -c 'set -a && . ./.env.local && set +a && SMOKE_BASE_URL=https://listmyproduct.vercel.app pnpm test:smoke:prod'
 */
...
const BASE_URL = (process.env.SMOKE_BASE_URL ?? "https://listmyproduct.vercel.app").replace(/\/$/, "");
...
async function main() {
  console.log(`\nListMyProduct production smoke — ${BASE_URL}\n`);
