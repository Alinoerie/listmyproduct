...
export async function POST(_req: NextRequest) {
  ...
  if (isDigestEmailConfigured()) {
    ...
    if (apiKey) {
      ...
      const from = process.env.EMAIL_FROM ?? "ListMyProduct <onboarding@resend.dev>";
      const emailHtml = buildWeeklyDigestEmail(digest, STUDIO_URL);
      await resend.emails.send({
        from,
        to: digest.userEmail,
        subject: `Your ListMyProduct weekly digest — ${weekStart.toLocaleDateString("en-US", { month: "short", day: "numeric" })}`,
        html: emailHtml,
      });
