...
async function generateModuleAsset(params: {
  ...
  let imageUrl = await uploadBufferToCloudinary(buffer, `listmyproduct/${userId}/${productId}/aplus`);
  ...
  if (qaScore < 7 && !scaleCritical) {
    ...
    buffer = await postProcessAplusImage(buffer, postSpec);
    imageUrl = await uploadBufferToCloudinary(buffer, `listmyproduct/${userId}/${productId}/aplus`);
