...
async function generateModuleAsset(params: {
  ...
  let imageUrl = await uploadBufferToCloudinary(buffer, `listmyproduct/${userId}/${productId}`);
  ...
  if (qaScore < 7 && !scaleCritical) {
    ...
    imageUrl = await uploadBufferToCloudinary(buffer, `listmyproduct/${userId}/${productId}`);
